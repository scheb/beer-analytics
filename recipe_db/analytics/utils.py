...
class Trending:
    ...
    def get_trending_series(
        self, df: DataFrame, series_column: str, time_column: str, percent_column: str, count_column: str
    ) -> list:
        ...
        start_time = (
            pd.Timestamp("now").floor("D") - pd.offsets.MonthBegin(1) - pd.DateOffset(months=self.trending_window)
        )
        ...
        trend_df = recent_df.groupby(series_column).agg(
            {percent_column: ["mean", slope], count_column: "sum", time_column: "max"}
        )
        ...
        trend_df = trend_df[
            trend_df["data_points"] > self.trending_window
        ]  # At least one data point per time frame on average
        ...
        lookback_mean = lookback_mean.reindex(
            recent_series_ids, fill_value=0
        )  # Make sure we have values for all series
